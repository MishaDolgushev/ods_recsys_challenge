...
def fit_cv_model(dataset, model, splitter, top_n, metrics):
    ...
    for fold, (train_ids, test_ids, _) in enumerate(
        ...
        context = test_fold_context if model.require_recommend_context else None
...
def hot_pipeline(cfg):
    ...
    result_model.save(f"output/models/{cfg.model_name}.pkl")

    output_model = OutputModel(task=task, name=f"{cfg.model_name}")
    output_model.update_weights(f"output/models/{cfg.model_name}.pkl")
    task.close()
